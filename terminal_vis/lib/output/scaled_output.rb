...
# @Last Modified time: 2016-02-19 14:15:30

module DataOutput

  class ScaledOutput < Base
    ...
    def self.print_dataset(data_set, meta_data, options)
      ...
      print_data(options[:legend], @scale_meta.domain_x, @scale_meta.domain_y)
    end
    ...
    def self.prepare_attributes(data_set, meta_data, options)
      ...
      @scale_meta = calculate_resolution_meta(meta_data)
    ...
    def self.print_meta_information
      puts "\nDataset: #{@scale_meta.name}"

      print_domain_information(@scale_meta.domain_x, "\nX")
      print_domain_information(@scale_meta.domain_y, 'Y')

      if (@scale_meta.domain_z != nil)
        print_domain_information(@scale_meta.domain_z, 'Z')
      end
    end
    ...
    def self.calculate_resolution_meta(meta_data)
      ...
      meta_string.concat(add_domain_information(meta_data.domain_x, delta_x))
    ...
    def self.determine_coordinates
      ...
      coordinates[:x] = @scale_meta.domain_x.lower +
                        calculated_dimension_delta(@scale_meta.domain_x)
      coordinates[:y] = @scale_meta.domain_y.lower +
                        calculated_dimension_delta(@scale_meta.domain_y)
      return coordinates
    end
    ...
    def self.determine_values
      { :inter_x => calculated_dimension_delta(@scale_meta.domain_x),
        :delta_x => @scale_meta.domain_x.step,
        :inter_y => calculated_dimension_delta(@scale_meta.domain_y),
        :delta_y => @scale_meta.domain_y.step }
    end
